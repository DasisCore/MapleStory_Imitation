...
{
	if (nullptr == CSoundMgr::GetInst()->GetSoundDevice()) assert(nullptr);		// 사운드 객체 생성되지 않음.
	...
	{
		if (false == LoadWaveSound(_strPath)) assert(nullptr);
	}
	else assert(nullptr);
...
bool CSound::LoadWaveSound(const wstring& _strPath)
{
	...
	if (pWrite1 != NULL) mmioRead(hFile, (char*)pWrite1, dwlength1);
	if (pWrite2 != NULL) mmioRead(hFile, (char*)pWrite2, dwlength2);
...
{
	// Play 함수의 1번째 2번째 인자는 0으로 이미 예약되어있음.
	// 3번재 변수는 사운드를 반복재생 할 것인지 아닌지를 결정한다.
	if (_bLoop)
	{
...
void CSound::PlayToBGM(bool _bLoop)
{
	...
	// Play 함수의 1번째 2번째 인자는 이미 0으로 예약되어있음.
...
{
	if (_fVolume > 100.f) _fVolume = 100.f;
	else if (_fVolume <= 0.f) _fVolume = 0.00001f;

	// 1 ~ 100 사이값을 데시벨 단위로 지정
